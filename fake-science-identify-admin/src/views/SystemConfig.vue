...
const loadSystemSettings = async () => {
  try {
    const { data } = await request.get('/admin/system/settings')
...
const loadBackupSettings = async () => {
  try {
    const { data } = await request.get('/admin/backup/settings')
    if (data && typeof data === 'object') {
      Object.assign(backupForm, data)
    }
  } catch (e) {
    console.error('加载备份策略失败', e)
  }
}
...
const handleClearCache = async (type) => {
  ...
  try {
    await ElMessageBox.confirm(`确定要清理${typeText}吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
  ...
  try {
    const res = await request.post('/admin/system/clear-cache', { type })
    const body = res?.data
    if (type === 'redis') {
      if (body?.redis_available === false) {
        ElMessage.warning(body.message || 'Redis 未连接，推理缓存未使用 Redis')
        return
      }
      if (body?.flushed) {
        ElMessage.success(body.message || `${typeText}已清理`)
        return
      }
    }
    ElMessage.success(body?.message || `${typeText}操作完成`)
  } catch (e) {
